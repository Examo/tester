...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public function preanswer($answer)
    {
        if (!$this->isFinished()) {
            $this->storeAnswer($answer);
    ...
    {
        $this->useComment();
        return $this->getCurrentQuestion()->getComment( true );
    }
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    protected function useComment()
    {
        ...
        $comments[$question] = true;

        \Yii::$app->session->set($this->getSessionKey('comments'), $comments);
    }
    ...
    public function isCommentUsed() {
        $queue = $this->getQueue();
        $question = $queue[$this->getCurrentQuestionNumber()];

        $comments = $this->getComments();

        return isset($comments[$question]) && $comments[$question];
    }
