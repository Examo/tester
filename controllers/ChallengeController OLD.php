        ...
        if (count($session->getAnswers())) {
            ...
            if (!Yii::$app->user->isGuest) {
                ...
                foreach ($lastChallengeQuestions as $lastChallengeQuestion) {
                    $lastChallengeQuestionCost = Question::find()->where(['id' => $lastChallengeQuestion->question_id])->one();
                    // надо доставать тип задания и умножать его на три, если это "3 вопроса"
                    \yii\helpers\VarDumper::dump($lastChallengeQuestionCost->question_type_id, 10, true);
                    if ($lastChallengeQuestionCost->question_type_id == 8){
                        $allLastChallengeQuestionsCost += $lastChallengeQuestionCost->cost * 3;
                        //print '8 тип задания: ';
...
//                            }
            ...
            //$lastChallengeQuestionCost = Question::find()->where(['id' => 49])->one();
            //\yii\helpers\VarDumper::dump($lastChallengeQuestionCost->question_type_id, 10, true);
            if (Yii::$app->user->isGuest) {
                $testQuestions = $summary->getQuestions();
                $testResults = $summary->getCorrectness();
